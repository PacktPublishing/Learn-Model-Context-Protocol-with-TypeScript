     ...
     if(isJsonRpcMessage(line)) {
       ...
       switch(message.method) {
            case "tools/list":
                ...
                break;
            case "tools/call":
                let toolResponse = `Called tool ${message.params.name} with arguments ${JSON.stringify(message.params.args)}`;
              
                let callToolResponse = {
                    "jsonrpc": "2.0",
                    "id": message.id,
                    "result": {
                        "properties": {
                ...
                console.log(JSON.stringify(callToolResponse));
                break;
            default:
